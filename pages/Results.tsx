      ...
      <div className="bg-white rounded-[23px] flex flex-col h-[320px]">
        ...
        <div ref={scrollRef} className="flex-grow overflow-y-auto p-3 space-y-3 hide-scrollbar">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[90%] px-3 py-2 rounded-xl text-[11px] font-bold ${m.role === 'user' ? 'bg-orange-500 text-white' : 'bg-slate-100 text-slate-700 border border-slate-200'}`}>
                {m.content}
              </div>
            </div>
          ))}
          {isLoading && <div className="flex gap-1 p-1"><span className="w-1 h-1 bg-orange-400 rounded-full animate-bounce"></span><span className="w-1 h-1 bg-orange-400 rounded-full animate-bounce delay-75"></span><span className="w-1 h-1 bg-orange-400 rounded-full animate-bounce delay-150"></span></div>}
        </div>
        ...
        <div className="p-3 border-t border-slate-100">
          <div className="flex gap-1 items-center bg-slate-50 p-1 rounded-xl border border-slate-200">
            ...
            <button onClick={() => handleAISend(input)} className="w-7 h-7 bg-orange-500 text-white rounded-lg flex items-center justify-center hover:bg-orange-600 transition-transform active:scale-90"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg></button>
          </div>
        </div>
      </div>
    </div>
    ...
    return (
      <div className="space-y-8 p-1">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <select 
              ...
              className="w-full px-3 py-2.5 text-xs font-bold bg-slate-50 border border-slate-200 rounded-xl focus:border-orange-500 focus:ring-0 outline-none transition-all"
            >
          ...
          {filters.district && (
            <div className="space-y-2 animate-fadeIn">
              ...
              <div className="max-h-48 overflow-y-auto pr-2 space-y-1.5 scrollbar-thin scrollbar-thumb-orange-200">
                {neighborhoodOptions.map(n => (
                  <label key={n} className="flex items-center gap-3 cursor-pointer group py-1 px-2 hover:bg-orange-50 rounded-lg transition-colors">
                    <input 
                      type="checkbox" 
                    ...
                    />
                    <span className="text-xs font-bold text-slate-600 group-hover:text-orange-600 transition-colors">{n}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 gap-2">
            {['Satılık', 'Kiralık', 'Günlük Kiralık'].map(type => (
              <button 
                ...
                className={`py-2.5 px-4 text-[11px] font-black uppercase tracking-wider rounded-xl border-2 text-left transition-all ${filters.dealType === type ? 'bg-orange-500 border-orange-500 text-white shadow-lg shadow-orange-500/20' : 'bg-white border-slate-100 text-slate-500 hover:border-orange-100'}`}
              >
                {type}
              </button>
            ))}
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <input type="number" placeholder="Min" value={filters.minPrice || ''} onChange={(e) => onFilterChange({ ...filters, minPrice: e.target.value ? parseInt(e.target.value) : undefined })} className="w-full px-3 py-2.5 text-xs font-bold border-2 border-slate-100 rounded-xl outline-none focus:border-orange-500 bg-slate-50 transition-all" />
            <input type="number" placeholder="Max" value={filters.maxPrice || ''} onChange={(e) => onFilterChange({ ...filters, maxPrice: e.target.value ? parseInt(e.target.value) : undefined })} className="w-full px-3 py-2.5 text-xs font-bold border-2 border-slate-100 rounded-xl outline-none focus:border-orange-500 bg-slate-50 transition-all" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {ROOM_COUNTS.map(rc => (
              <button key={rc} onClick={() => onFilterChange({ ...filters, roomCount: filters.roomCount === rc ? undefined : rc })} className={`py-2 text-[10px] font-black rounded-lg border-2 transition-all ${filters.roomCount === rc ? 'bg-orange-500 border-orange-500 text-white shadow-md' : 'bg-white border-slate-100 text-slate-500 hover:border-orange-50'}`}>{rc}</button>
            ))}
          </div>
        </div>

        <button onClick={() => onFilterChange({})} className="w-full py-4 text-[11px] font-black uppercase tracking-widest text-orange-600 bg-orange-50 hover:bg-orange-100 rounded-2xl transition-all active:scale-95 shadow-sm">Tüm Filtreleri Sıfırla</button>
      </div>
    );
  };
        ...
        <main className="flex-grow">
          <div className="hidden md:flex items-center justify-between mb-8 pb-4 border-b border-slate-100">
            <div className="text-sm font-medium text-slate-500">
              <span className="font-black text-slate-900 px-2 py-1 bg-orange-50 rounded-lg text-orange-600 mr-2">{sortedListings.length}</span> ilan kriterlerinle eşleşiyor
            </div>
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortOption)} className="px-4 py-2 border-2 border-slate-100 rounded-xl text-xs font-black uppercase tracking-wider bg-white focus:outline-none focus:border-orange-500 transition-all cursor-pointer">
          ...
          {sortedListings.length > 0 ? (
              ...
              {sortedListings.map((listing) => (
                <div key={listing.id} onClick={() => navigate(`/details/${listing.id}`)} className="cursor-pointer group flex flex-col">
                  ...
                  <div className="flex-grow space-y-2 px-1">
                    <h3 className="font-black text-[15px] leading-tight text-slate-900 line-clamp-2 uppercase tracking-tight group-hover:text-orange-600 transition-colors">{listing.neighborhood}, {listing.location}</h3>
                    ...
                    <div className="pt-2 flex items-baseline gap-2">
                      <span className="text-2xl font-black text-orange-600 tracking-tighter">{listing.price.toLocaleString('tr-TR')}</span>
            ...
            <div className="flex flex-col items-center justify-center py-40 border-4 border-dotted border-slate-100 rounded-[60px] bg-slate-50/50">
              ...
              <button onClick={() => onFilterChange({})} className="px-12 py-5 bg-orange-600 text-white font-black rounded-2xl shadow-2xl shadow-orange-600/30 active:scale-95 transition-all">Filtreleri Sıfırla</button>
            </div>
          )}
        </main>
      </div>
        ...
        <div className="fixed inset-0 z-[200] md:hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 top-10 bg-white rounded-t-[40px] overflow-hidden flex flex-col shadow-2xl animate-slideUp">
            <div className="flex items-center justify-between p-8 border-b border-slate-100">
              ...
              <button onClick={() => onFilterChange({})} className="text-xs font-black uppercase text-orange-600 underline">Sıfırla</button>
            </div>
            <div className="flex-grow overflow-y-auto p-10"><SidebarContent /></div>
            <div className="p-8 bg-slate-50 border-t border-slate-100">
              <button onClick={() => setIsMobileFilterOpen(false)} className="w-full py-5 bg-orange-600 text-white font-black rounded-2xl shadow-xl shadow-orange-600/30">Sonuçları Listele</button>
            </div>
          </div>
        </div>
      )}
