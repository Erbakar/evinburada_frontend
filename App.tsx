    ...
    <div className="min-h-screen flex flex-col relative bg-white">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-[100]">
        <div className="max-w-[1760px] mx-auto px-6 md:px-10 lg:px-20 h-20 flex items-center justify-between">
          ...
          >
            <span className="text-[10px] font-medium uppercase tracking-widest text-slate-700 group-hover:text-brand-500">Bölge</span>
            <div className="w-[1px] h-4 bg-slate-200"></div>
            <span className="text-[10px] font-medium uppercase tracking-widest text-slate-700 group-hover:text-brand-500">İlan Tipi</span>
            <div className="w-[1px] h-4 bg-slate-200"></div>
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-medium uppercase tracking-widest text-slate-400">Filtrele</span>
          ...
          <div className="flex items-center gap-4">
            <span className="hidden lg:block text-[10px] font-medium uppercase tracking-widest text-slate-600 hover:text-brand-600 cursor-pointer transition-colors">Giriş Yap</span>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-[40px] shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between">
              ...
              <h2 className="text-lg font-medium tracking-tighter uppercase">Filtreleme</h2>
              <button onClick={() => setModalFilters({})} className="text-xs font-medium uppercase text-brand-600">Sıfırla</button>
            </div>
            
            <div className="flex-grow overflow-y-auto p-10 space-y-12 hide-scrollbar">
              <section className="space-y-4">
                <h3 className="text-xs font-medium uppercase tracking-widest text-slate-400">İlan Tipi</h3>
                <div className="grid grid-cols-3 gap-3">
                  {['Satılık', 'Kiralık', 'Günlük Kiralık'].map(type => (
                    <button 
                      key={type}
                      onClick={() => setModalFilters(p => ({...p, dealType: type as any}))}
                      className={`py-4 rounded-2xl border-2 font-medium text-[10px] uppercase tracking-widest transition-all ${modalFilters.dealType === type ? 'border-brand-500 bg-brand-50 text-brand-600' : 'border-slate-100 hover:border-brand-100 text-slate-600'}`}
                    >
                      {type}
                    </button>
                  ))}
              ...
              <section className="space-y-4">
                <h3 className="text-xs font-medium uppercase tracking-widest text-slate-400">Konum Seçimi</h3>
                <div className="flex gap-2 mb-4">
                   {DISTRICTS.map(d => (
                     <button key={d} onClick={() => setSelectedDistrict(d)} className={`px-5 py-2 rounded-full text-[10px] font-medium uppercase tracking-widest transition-all ${selectedDistrict === d ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>{d}</button>
                   ))}
                </div>
                <div className="grid grid-cols-2 gap-2">
                   {NEIGHBORHOODS[selectedDistrict].map(n => (
                     <button 
                       ...
                       className={`px-4 py-3 rounded-xl border text-left text-xs font-medium transition-all flex items-center justify-between ${modalFilters.locations?.includes(n) ? 'border-brand-500 bg-brand-50 text-brand-600' : 'border-slate-100 text-slate-600 hover:border-brand-100'}`}
                     >
                       {n}
            ...
            <div className="p-8 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-6">
              <button onClick={() => setIsFilterModalOpen(false)} className="text-xs font-medium uppercase tracking-widest text-slate-400">İptal</button>
              <button onClick={applyModalFilters} className="px-12 py-5 bg-brand-600 text-white font-medium rounded-2xl shadow-xl hover:bg-brand-dark transition-all active:scale-95" style={{boxShadow: '0 20px 25px -5px rgba(250, 1, 117, 0.2)'}}>Uygula</button>
            </div>
          </div>
        </div>
      )}
      ...
      <footer className="bg-white border-t border-slate-100 py-10">
        <div className="max-w-[1760px] mx-auto px-6 md:px-10 lg:px-20 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] text-slate-400 font-medium uppercase tracking-widest">© 2026 Tüm hakları saklıdır.</span>
          </div>

          <div className="flex items-center gap-8 text-[10px] font-medium uppercase tracking-widest text-slate-500">
